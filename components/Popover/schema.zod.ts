...
match the severity. */
  ...
  appendTo: z.enum(['inline']).optional().default('() => document.body'),
...
Usage: <Popover><Button>Reference</Button></Popover> */
  ...
  closeBtnAriaLabel: z.string().optional().default('Close'),
  /** Distance of the popover to its target. Defaults to 25. */
  distance: z.number().optional().default(25),
...
footerContent={hide => <Button onClick={() => hide()}>Close</Button>} */
  footerContent: z.any().optional().default('null'),
  /** Removes fixed-width and allows width to be defined by contents. */
  hasAutoWidth: z.boolean().optional().default(false),
  ...
  headerComponent: z.enum(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']).optional().default('h6'),
...
popover close button is clicked, the enter key was used on it, or the escape key is used. */
  shouldClose: z.any().optional().default('(): void => null'),
  /** Callback function that is only invoked when isVisible is also controlled. Called when the
enter key is used on the focused trigger. */
  shouldOpen: z.any().optional().default('(): void => null'),
  ...
  triggerAction: z.enum(['click', 'hover']).optional().default('click'),
